...
test('wish-fulfilled submit keeps the design field hidden', async ({ page }) => {
  await page.goto('/schedule-a-session?service=embodied-manifestation');
...
test('header book-a-session CTA hides design field for manifestation', async ({ page }) => {
  await page.goto('/services/embodied-manifestation');
  await page.getByRole('banner').getByRole('link', { name: /Book a Session/i }).click();
  await expect(page).toHaveURL(/schedule-a-session\?service=embodied-manifestation/);
  await expect(page.locator('#schedule-design-wrap')).toBeHidden();
});

test('fine-art booking still shows the design field', async ({ page }) => {
  await page.goto('/schedule-a-session?service=fine-art-body-painting');

  await expect(page.locator('#schedule-session-type')).toHaveValue('Fine Art Body Painting');
  await expect(page.locator('#schedule-design-wrap')).toBeVisible();
  await expect(page.getByPlaceholder(/Van Gogh/i)).toBeVisible();
});
